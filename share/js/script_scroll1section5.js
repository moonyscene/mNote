/**
 * @File Name : script_scroll1section5.js
...
(function(){
	...
	$(function(){
		var $my, // 활성 섹션내비 담는거
			...
			$nav1top = $('#gotop1'),	// 위로가기
			$mw1nav = $('.mousewheel1[class*="scroll1"]'), // 마우스휠내비
		 ...
		 */
			...
			$mw1nav.each(function(index, element){ // 마우스휠내비
				$(this).on('click', function(e){
					e.preventDefault();
		...
		/** ◇◆ 스크롤. 섹션 위치로 가면 해당 섹션내비 활성. 20180327~. 20200205. MoonYoungshin
		 */
		$(window).on('scroll load resize', function(e){
			...
			if(nowScrollTop > ($section.eq(7).offset().top - vmTop)){
				onSN = 8;
			}
			if(nowScrollTop > ($section.eq(8).offset().top - vmTop)){
				onSN = 9;
			}
			// 맨끝은 ★☆
			// ★ 반이상 보이거나 푸터보이기시작하면 활성 //  20180327.
			//if( (nowScrollTop > ($section.eq(8).offset().top - vmTop2)) || (nowScrollTop > vmTopLast) ){
			//	onSN = 9;
			//}
		...
		/** ◇◆  마우스휠. 20180327. 20190529. 20200205. MoonYoungshin
		 * 20190529 [CR73~] 마우스 휠 스크롤 버벅 결함 해결!
		 */
		(function(){
			var delta = 0,
				endScrollTop = 0; // 휠 목적지 맨끝
			...
			$("html, body").on('mousewheel DOMMouseScroll', function(e) {
				...
				endScrollTop = $section.eq(8).offset().top; // 아홉번째 섹션 상단 위치
		...
		(function(){
			...
			//var count = 0;
			var $my = $('[id^="mainbody"]:not(#mainbody9)');
			$my.each(function(){
